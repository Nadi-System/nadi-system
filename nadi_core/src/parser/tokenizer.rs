...
impl<'a> VecTokens<'a> {
    pub fn new(tokens: Vec<Token<'a>>) -> Result<Self, TaskParseError> {
        if let Some((ind, tk)) = tokens.iter().enumerate().find(|(_, t)| !t.ty.is_valid()) {
            let mut line = 0;
            let mut col = 0;
            let mut linestr = Vec::new();
            for t in &tokens[..ind] {
                match t.ty {
                    TaskToken::NewLine => {
                        line += 1;
                    ...
                    _ => {
                        col += t.content.len();
                        linestr.push(&t.content);
                    }
                }
            }
            // string from the reminder of the line
            for t in &tokens[ind..] {
                match t.ty {
                    TaskToken::NewLine => break,
                    _ => {
                        linestr.push(&t.content);
                    }
                }
            }
            let linestr: Vec<&str> = linestr.into_iter().map(|&s| s).collect();
            Err(TaskParseError {
                ty: ParseErrorType::InvalidToken,
                line,
                col,
