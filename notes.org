...
trait Eval: Display + Clone {
  fn eval(self, ctx: &Ctx);
  fn eval_mut(self, ctx: &mut Ctx) {
   self.eval(ctx)
}
}
...
So far I have been waiting on having a .nadi file because NADI should have both network and the attributes and series/timeseries informations.

I think we're going to use .nadi format to represent a zip file with multiple files inside it.

The .nadi file will contain:
- network.net file with the network information,
- attrs/<node>.toml files with attributes for all the nodes,
- series/<node>/<name>.csv file with series value for each node (maybe we should have data in binary format instead of text format??)
- timeseries/<node>/<name>.csv file with timeseries values.

This way you can save a network from a task context in its complete form, and reload it as well.

TimeSeries is a complicated part because we are using shared references to denote shared timeline. So maybe we do:

- timelines/<name>.txt
- timelines/<name>.toml with node names as keys and timeseries names with this timeline as values
- timeseries/<node>/<name>.txt now only has values

If we combine this with the WASM based tasks. then we can serve .nadi files in the web and have all the network analysis on client side computers. We just have to write things in a way that people don't need plugins. Or they use plugins only to generate .nadi files.

Once I am done with reports on NADI, maybe I should work on the WASM compatible nadi language, and let go of the plugin system for now. Use full rust types. Maybe find a different way to write plugins. The plugins can be a bit slow if they need fancy filetypes and such. And they can be fast if they are wasm based?? Or plugins are only supported in the desktop version and not web version.
